...
inline const char* errorCodeName(ErrorCode code) {
    switch (code) {
        ...
        case ErrorCode::eNotImplemented:   return "eNotImplemented";
        case ErrorCode::eOutOfMemory:      return "eOutOfMemory";
        case ErrorCode::eFileNotFound:     return "eFileNotFound";
        case ErrorCode::eFileOpenFailed:   return "eFileOpenFailed";
        case ErrorCode::eFileReadFailed:   return "eFileReadFailed";
        case ErrorCode::eFileWriteFailed:  return "eFileWriteFailed";
        case ErrorCode::ePathInvalid:      return "ePathInvalid";
        case ErrorCode::eUnsupportedFormat:  return "eUnsupportedFormat";
        case ErrorCode::eUnsupportedFeature: return "eUnsupportedFeature";
        case ErrorCode::eParseError:       return "eParseError";
        case ErrorCode::eDataCorrupt:      return "eDataCorrupt";
        case ErrorCode::eDataTruncated:    return "eDataTruncated";
        case ErrorCode::eInvalidDimensions: return "eInvalidDimensions";
        case ErrorCode::eInvalidPixelType: return "eInvalidPixelType";
        case ErrorCode::eThirdPartyError:  return "eThirdPartyError";
    }
    return "?";
}
...
inline void printStatus(const Status& s) {
    if (s.ok()) {
        ...
        if (!s.subsystem.empty()) oss << "  subsystem: " << s.subsystem;
        if (!s.message.empty())   oss << "  msg: "       << s.message;
        if (!s.path.empty())      oss << "  path: "      << s.path;
...
inline const char* pixelTypeName(vne::image::VolumePixelType t) {
    ...
    switch (t) {
        case T::eUint8:   return "uint8";
        case T::eInt8:    return "int8";
        case T::eUint16:  return "uint16";
        case T::eInt16:   return "int16";
        case T::eUint32:  return "uint32";
        case T::eInt32:   return "int32";
        case T::eFloat32: return "float32";
        case T::eFloat64: return "float64";
        case T::eUnknown: return "unknown";
    }
    return "?";
}
...
inline void printVolumeInfo(const vne::image::Volume& v, const char* label = "Volume") {
    ...
    VNE_LOG_INFO << "  dir:     ["
        ...
        << v.direction[3] << ", " << v.direction[4] << ", " << v.direction[5] << " | "
        << v.direction[6] << ", " << v.direction[7] << ", " << v.direction[8] << "]";
    VNE_LOG_INFO << "  type:    " << pixelTypeName(v.pixel_type)
...
inline void printMeshInfo(const vne::mesh::Mesh& m, const char* label = "Mesh") {
    VNE_LOG_INFO << label << ":";
    VNE_LOG_INFO << "  vertices: " << m.getVertexCount()
        << "  indices: " << m.getIndexCount()
        << "  submeshes: " << m.getSubmeshCount()
        << "  materials: " << m.getMaterialCount();
    VNE_LOG_INFO << "  aabb_min: ("
        << m.aabb_min[0] << ", " << m.aabb_min[1] << ", " << m.aabb_min[2] << ")";
    VNE_LOG_INFO << "  aabb_max: ("
        << m.aabb_max[0] << ", " << m.aabb_max[1] << ", " << m.aabb_max[2] << ")";
    VNE_LOG_INFO << "  normals: " << (m.has_normals ? "yes" : "no")
        << "  tangents: " << (m.has_tangent ? "yes" : "no")
