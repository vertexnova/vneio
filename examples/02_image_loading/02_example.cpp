...
int runImageLoadingExample() {
    ...
    printSection("Load image from file");
    {
        ...
        if (!check(!img.isEmpty(),   "image is not empty"))           return 1;
        if (!check(img.getWidth()  > 0, "width > 0"))                 return 1;
        if (!check(img.getHeight() > 0, "height > 0"))                return 1;
        if (!check(img.getChannels() >= 1 && img.getChannels() <= 4,
                   "channels in [1,4]"))                               return 1;
        if (!check(img.getData() != nullptr, "getData() non-null"))    return 1;

        VNE_LOG_INFO << "  width=" << img.getWidth()
    ...
    printSection("Construct Image from file path");
    {
        ...
        if (!check(!img.isEmpty(), "Image(path) is not empty")) return 1;
    }
    ...
    {
        vne::image::Image img;
        if (!check(img.loadFromFile(tex_path), ("loadFromFile: " + tex_path).c_str())) return 1;

        if (!check(img.resize(128, 128),     "resize(128,128) returned true")) return 1;
        if (!check(img.getWidth()  == 128,   "width == 128 after resize"))      return 1;
        if (!check(img.getHeight() == 128,   "height == 128 after resize"))     return 1;

        const std::string out_path = tmpPath("vneio_ex02_resized.png");
        if (!check(img.saveToFile(out_path, "png"),
                   ("saveToFile: " + out_path).c_str())) return 1;

        vne::image::Image reloaded;
        if (!check(reloaded.loadFromFile(out_path, false), "reload saved PNG"))  return 1;
        if (!check(reloaded.getWidth()  == 128, "reloaded width == 128"))        return 1;
        if (!check(reloaded.getHeight() == 128, "reloaded height == 128"))       return 1;
    ...
    printSection("Flip vertically");
    {
        ...
        if (!check(img.loadFromFile(tex_path, false), ("loadFromFile (no flip): " + tex_path).c_str()))
            return 1;
        img.flipVertically();
        if (!check(!img.isEmpty(), "flipped image is not empty")) return 1;
    }
    ...
    printSection("image_utils raw load / free");
    {
        ...
        if (!check(raw != nullptr,           "loadImage returned non-null")) return 1;
        if (!check(w > 0 && h > 0 && c > 0, "w/h/c all > 0"))              return 1;
        ...
        if (!check(vne::image::image_utils::saveImage(raw_out, raw, w, h, c, "png"),
                   ("image_utils::saveImage: " + raw_out).c_str())) return 1;

        vne::image::image_utils::freeImage(raw);
        if (!check(true, "freeImage (no crash)")) return 1;
    }
    ...
    printSection("Construct Image from raw data buffer");
    {
        ...
        if (!check(!img.isEmpty(),          "Image(raw) is not empty")) return 1;
        if (!check(img.getWidth()    == W,  "width matches"))           return 1;
        if (!check(img.getHeight()   == H,  "height matches"))          return 1;
        if (!check(img.getChannels() == CH, "channels matches"))        return 1;
    }
    ...
    printSection("Error path: non-existent file");
    {
        ...
        if (!check(!ok,         "loadFromFile returns false for missing file")) return 1;
        if (!check(img.isEmpty(),"isEmpty()==true after failed load"))          return 1;
    }
