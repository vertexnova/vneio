...
int runMeshLoadingExample() {
    ...
    printSection("Load minimal.stl via AssimpLoader::loadFile");
    {
        ...
        if (!ok) {
            VNE_LOG_WARN << "minimal.stl not found: " << loader.getLastError() << " — skipping mesh tests";
    ...
    printSection("Load with AssimpLoaderOptions (normals on, no normalize)");
    {
        ...
        if (!mesh.vertices.empty()) {
            ...
            VNE_LOG_INFO << "  first vertex pos=(" << v.position[0] << ", " << v.position[1] << ", " << v.position[2]
                         << ")";
        }
    }
    ...
    printSection("OBJ export round-trip");
    {
        ...
        if (!check(vne::mesh::exportObj(obj_path, mesh, opts, &err), ("exportObj: " + obj_path).c_str())) {
        ...
        if (!check(reload_loader.loadFile(obj_path, reloaded), ("reload OBJ: " + obj_path).c_str())) {
