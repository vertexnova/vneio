...
|-----------|-------|---------|
| Classes/Structs | PascalCase | `Buffer`, `Image` |
| Interface classes | `I` + PascalCase | `IImageLoader`, `IMeshLoader` |
| Enums | PascalCase | `ErrorCode`, `VolumePixelType` |
| Enum values | `e` + PascalCase + explicit value | `eNone = 0`, `eUint8 = 1` |
| Functions/Methods | camelCase | `loadImage()`, `exportNrrd()` |
| Constants | `k` + PascalCase | `kMaxBufferSize` |
| Private/Protected members | snake_case + `_` | `buffer_size_`, `is_initialized_` |
| Macros | ALL_CAPS with `VNE_` prefix where applicable | `VNEIO_API`, `VNE_BUILD_TYPE_DEBUG` |
| Namespaces | lowercase | `vne`, `vne::io`, `vne::image` |
| File names | snake_case | `image_loader.h` |
...
## This repository (VneIo)

### Project layout

- Library sources: **src/vertexnova/io/** — Public API: **include/vertexnova/io/** — Tests: **tests/** — Test data: **testdata/**.
- Dependencies: **deps/internal/** (e.g. vnelogging, vnecommon), **deps/external/** (e.g. googletest, assimp, stb). No top-level `external/` or `libs/`.

### Formatting and static analysis

- **.clang-format** and **.clang-tidy** are enforced in CI; focus review on design and logic. Flag style/naming if automation might miss it.
