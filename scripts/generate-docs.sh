...
check_prerequisites() {
    ...
    if [[ ! -f "$PROJECT_ROOT/CMakeLists.txt" ]]; then
        log_error "Not in VneIo project root."
        exit 1
    fi
...
generate_api_docs() {
    ...
    cmake -S "$PROJECT_ROOT" -B "$BUILD_DIR" \
        ...
        -DENABLE_DOXYGEN=ON \
        -DVNEIO_LIB_TYPE=shared \
        -DVNEIO_BUILD_TESTS=OFF

    local target=""
    if cmake --build "$BUILD_DIR" --target help 2>/dev/null | grep -q vneio_doc_doxygen; then
        target="vneio_doc_doxygen"
...
show_help() {
    echo "VneIo documentation helper"
    echo ""
