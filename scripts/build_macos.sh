...
if [[ "$GENERATE_XCODE" == true ]]; then
  CONFIGURE_CMD="cmake -G Xcode -DCMAKE_BUILD_TYPE=$BUILD_TYPE -DCMAKE_C_COMPILER=${CC_PATH} -DCMAKE_CXX_COMPILER=${CXX_PATH} -DCMAKE_OSX_DEPLOYMENT_TARGET=13.3 -DVNEIO_BUILD_TESTS=ON -DVNEIO_BUILD_EXAMPLES=OFF $PROJECT_ROOT"
  ...
  CONFIGURE_CMD="cmake -DCMAKE_BUILD_TYPE=$BUILD_TYPE -DCMAKE_C_COMPILER=${CC_PATH} -DCMAKE_CXX_COMPILER=${CXX_PATH} -DCMAKE_OSX_DEPLOYMENT_TARGET=13.3 -DVNEIO_BUILD_TESTS=ON -DVNEIO_BUILD_EXAMPLES=OFF $PROJECT_ROOT"
