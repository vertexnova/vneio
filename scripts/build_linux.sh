...
interactive_mode() {
  echo "=== VneIo Interactive Build Configuration ==="
  echo ""
...
build_cmake_command() {
  if [ "$COMPILER" = "gcc" ]; then
    echo "cmake -DCMAKE_BUILD_TYPE=$BUILD_TYPE -DVNEIO_LIB_TYPE=$LIB_TYPE -DCMAKE_C_COMPILER=gcc -DCMAKE_CXX_COMPILER=g++ -DVNEIO_BUILD_TESTS=ON $IPO_FLAGS $PROJECT_ROOT"
  elif [ "$COMPILER" = "clang" ]; then
    echo "cmake -DCMAKE_BUILD_TYPE=$BUILD_TYPE -DVNEIO_LIB_TYPE=$LIB_TYPE -DCMAKE_C_COMPILER=clang -DCMAKE_CXX_COMPILER=clang++ -DVNEIO_BUILD_TESTS=ON $IPO_FLAGS $PROJECT_ROOT"
  fi
}
